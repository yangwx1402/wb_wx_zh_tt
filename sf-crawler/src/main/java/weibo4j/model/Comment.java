...
public class Comment extends WeiboResponse {
	...
	private long source_mid;

	public long getSource_mid() {
		return source_mid;
	}

	public void setSource_mid(long source_mid) {
		this.source_mid = source_mid;
	}
	...
	public static CommentWapper constructWapperCommentsJson(String jsonString) throws WeiboException, JSONException {
		JSONObject json = new JSONObject(jsonString); //asJSONArray();
		try {
			JSONArray comments = json.getJSONArray("comments");
			int size = comments.length();
			List<Comment> comment = new ArrayList<Comment>(size);
